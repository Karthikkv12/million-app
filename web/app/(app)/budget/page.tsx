...
function NewEntryForm({ onDone }: { onDone: () => void }) {
  ...
  const mut = useMutation({
    mutationFn: () =>
      saveBudget({ type, category: cat, amount: parseFloat(amount), date, description: desc }),
    onSuccess: () => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 mb-4">
      <h3 className="font-bold text-sm text-gray-900 dark:text-white mb-3">New Entry</h3>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-3">
        ...
        <div>
          ...
          <input value={cat} onChange={(e) => setCat(e.target.value)} placeholder="General"
            className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100" />
        </div>
        <div>
...
export default function BudgetPage() {
  ...
  const filtered = typeFilter === "ALL"
    ...
    : [...entries]
        .filter((e) => e.type.toUpperCase() === typeFilter)
  ...
  return (
    <div className="p-4 max-w-screen-xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-black text-gray-900 dark:text-white">Budget</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4">
          <h2 className="text-sm font-bold text-gray-400 uppercase tracking-wide mb-3">Expense Breakdown</h2>
          {pieData.length === 0 ? (
                ...
                <tbody>
                  {filtered.map((e, i) => (
                    <tr key={i} className="border-b border-gray-50 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-gray-800/30">
                      <td className="px-3 py-2 text-gray-400 text-xs">{e.date.slice(0, 10)}</td>
                      <td className="px-3 py-2 font-semibold text-gray-900 dark:text-white">{e.category}</td>
                      ...
                      <td className="px-3 py-2 text-gray-400 text-xs truncate max-w-[120px]">{e.description ?? "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
